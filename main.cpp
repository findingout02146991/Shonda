...
orders *read_data_orders();
lineitem *read_data_lineitem();
result *select(customer *, orders *, lineitem *,char *,char *);
int run_shell(char *,char *,char *,int );
...
customer * read_data_customer()
{
	...
	if ((fp = fopen("/home/os_1/customer.txt", "r")) == NULL)
	{
		printf("1");
		return NULL;
	}
...
orders *read_data_orders()
{
	...
	if ((fp = fopen("/home/os_1/orders.txt", "r")) == NULL)
	{
		return NULL;
	}
	while (!feof(fp))
...
lineitem *read_data_lineitem()
{
	...
	if ((fp = fopen("/home/os_1/lineitem.txt", "r")) == NULL)
	{
		return NULL;
	}
	while (!feof(fp))
...
int run_shell(char *a,chr*b,char*c,int d)
{
	int i;
	customer *cus;
	orders *ord;
